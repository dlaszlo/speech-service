#!/usr/bin/env python3
"""Test WAV TTS audio streaming with OpenAI client."""
...
async def test_tts_audio_streaming():
    """Test WAV TTS audio streaming with OpenAI client and save to file."""
    
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(script_dir, "output")
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, "test_tts_audio_wav.wav")
    ...
    try:
        ...
        # Calculate duration roughly for WAV (ignoring header size for simplicity or exact calc)
        # WAV is 24kHz, 16bit, mono -> 48000 bytes/sec
