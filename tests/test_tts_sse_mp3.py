#!/usr/bin/env python3
"""Test MP3 TTS streaming with SSE events."""
...
def test_tts_streaming():
    """Test MP3 TTS streaming with SSE format and verify continuous streaming."""
    
    base_url = "http://localhost:8000"
    
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(script_dir, "output")
    output_file = os.path.join(output_dir, "test_tts_streaming_mp3.mp3")
    ...
    try:
        ...
        logger.info(f"Audio saved to: {output_file} (MP3 format from server chunks)")
